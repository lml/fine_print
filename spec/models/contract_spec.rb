...
describe FinePrint::Contract do
  ...
  it "can be published" do
    ...
    expect(contract.published?).to be_false
    expect(contract.version).to be_nil
    expect(contract.is_latest).to be_false
    contract.publish
    expect(contract.published?).to be_true
  ...
  it "can be modified after published with no user contracts yet" do
    ...
    contract.publish
    contract.reload
    expect(contract.update_attributes(content: 'booya')).to be_true
  end
  ...
  it "can't be modified after a user agrees" do
    ...
    contract.reload
    expect(contract.update_attributes(content: 'booya')).to be_false
  end
